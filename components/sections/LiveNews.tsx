...
type ScopeFilter = "all" | "ca-federal" | "ca-provinces" | "us" | "international";

const SCOPE_OPTIONS: { key: ScopeFilter; label: string }[] = [
  { key: "all", label: "All" },
  { key: "ca-federal", label: "Canada Federal" },
  { key: "ca-provinces", label: "CA Provinces" },
  { key: "us", label: "United States" },
  { key: "international", label: "International" },
];

type TopicFilter = "all" | "policy" | "construction" | "protests" | "market";
...
// Policy and Construction); the filter shows rows that match *any* of
...
const TOPIC_KEYWORDS: Record<Exclude<TopicFilter, "all">, RegExp> = {
  policy: /\b(bill|legislation|regulat|act\b|statute|law\b|laws\b|policy|executive order|rule\b|rules\b|framework|signed|vetoed|enacted|introduced|committee|parliament|assembly|directive|amendment|CMHC|Build Canada Homes)\b/i,
  "construction": /\b(development|permit|condo|vacancy|zoning|affordable|building permit|housing starts|new homes|units)\b/i,
  ...
  market: /\b(price|rent|mortgage|interest rate|housing market|sales|listings|inflation|affordability|HPI|home prices)\b/i,
};
...
const HIGH_SIGNAL = /\b(lawsuit|moratorium|upzon|rent freeze|eviction ban|housing emergency|rezoning|CMHC|HUD|executive order|injunction|ruling|court|passed|enacted|vetoed|signed|blocked|voided|halted|rejected|referendum)\b/gi;
const MED_SIGNAL = /\b(policy|regulat|legislat|development|permit|condo|vacancy|zoning|affordable|building permit|housing starts|bill\b|framework|protest|rally|opposition)\b/gi;
...
function isCanadaProvince(entity: Entity): boolean {
  return entity.region === "na" && entity.level === "state" && (entity.geoId?.startsWith("CA-") ?? false);
}

function isUsState(entity: Entity): boolean {
  return entity.region === "na" && entity.level === "state" && !entity.geoId?.startsWith("CA-");
}

function matchesScope(entity: Entity, s: ScopeFilter): boolean {
  if (s === "all") return true;
  if (s === "ca-federal") {
    return entity.id === "canada-federal";
  }
  if (s === "ca-provinces") {
    return isCanadaProvince(entity);
  }
  if (s === "us") {
    // Federal (us-federal) or US states.
    return (
      (entity.region === "na" && entity.level === "federal" && entity.geoId === "840") ||
      isUsState(entity)
    );
  }
  // international: anything not Canada or US.
  return !(
    entity.id === "canada-federal" ||
    isCanadaProvince(entity) ||
    (entity.region === "na" && entity.level === "federal" && entity.geoId === "840") ||
    isUsState(entity)
  );
}
...
export default function LiveNews({ showAll = false }: LiveNewsProps = {}) {
  ...
  const scopeCounts = useMemo(() => {
    ...
    for (const r of allRows) {
      if (matchesScope(r.entity, "ca-federal")) counts["ca-federal"] += 1;
      else if (matchesScope(r.entity, "ca-provinces")) counts["ca-provinces"] += 1;
      else if (matchesScope(r.entity, "us")) counts.us += 1;
  ...
  const topicCounts = useMemo<Record<TopicFilter, number>>(() => {
    ...
    for (const r of scoped) {
      ...
      if (matchesTopic(r.item, "construction")) counts["construction"] += 1;
      if (matchesTopic(r.item, "protests")) counts.protests += 1;
      if (matchesTopic(r.item, "market")) counts.market += 1;
    }
    return counts;
  }, [allRows, activeScope]);
