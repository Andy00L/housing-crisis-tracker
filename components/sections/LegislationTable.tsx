...
const CATEGORY_FILTER_LABEL: Record<CategoryFilter, string> = {
  ...
  rental: "Rental & Affordability",
  incentives: "Incentives & Investment",
  protections: "Protections & Services",
};

const CATEGORY_GROUP: Record<CategoryFilter, LegislationCategory[]> = {
  all: [],
  zoning: ["zoning-reform", "building-code", "transit-housing"],
  rental: ["rent-regulation", "affordable-housing", "property-tax"],
  incentives: ["development-incentive", "foreign-investment"],
  protections: ["tenant-protection", "homelessness-services"],
};
...
type JurisdictionFilter = "all" | "us-federal" | "us-states" | "ca-federal" | "ca-provinces" | "europe" | "asia-pacific";
...
function matchesJurisdiction(entity: Entity, j: JurisdictionFilter): boolean {
  ...
  if (j === "ca-provinces") {
    return entity.geoId?.startsWith("CA-") ?? false;
  }
  if (j === "europe") {
...
export default function LegislationTable({
  ...
  // Canada-first default. Users can widen to "All" or switch to US/Europe/Asia
  // with one click. Keeps the homepage table focused on Canadian bills on
  // initial load without hiding anything.
  const [activeJurisdiction, setActiveJurisdiction] =
    useState<JurisdictionFilter>("ca-federal");
  ...
  const jurisdictionCounts = useMemo(() => {
    ...
    for (const r of rows) {
      ...
      else if (matchesJurisdiction(r.entity, "ca-federal")) counts["ca-federal"] += 1;
      else if (matchesJurisdiction(r.entity, "ca-provinces")) counts["ca-provinces"] += 1;
