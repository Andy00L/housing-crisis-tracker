...
// Province name → entity geoId for entity lookup in census-division view.
const PROVINCE_GEOID_MAP: Record<string, string> = {
  "Alberta": "CA-AB", "British Columbia": "CA-BC", "Manitoba": "CA-MB",
  "New Brunswick": "CA-NB", "Newfoundland and Labrador": "CA-NL",
  "Northwest Territories": "CA-NT", "Nova Scotia": "CA-NS", "Nunavut": "CA-NU",
...
// the map on Canadian jurisdictions. Users can still click "United States"
// in the region selector to drop into the US states view.
...
export default function MapShell({
  ...
  const selectedEntity = useMemo((): Entity | null => {
    ...
    // Census division view: resolve CD entity or fall back to province entity.
    if (region === "na" && naView === "census-divisions" && selectedCduid) {
      const cd = getCensusDivisionByUid(selectedCduid);
      if (cd) return municipalityToEntity(cd);
    }
    if (region === "na" && naView === "census-divisions" && selectedProvinceName) {
      const geoId = PROVINCE_GEOID_MAP[selectedProvinceName];
      if (geoId) {
        const provEntity = getEntity(geoId, region);
  ...
  // ─── Canada provinces drill-down ──────────────────────────────────
  const handleViewProvinces = () => {
    navigateTo({ region: "na", naView: "provinces", selectedGeoId: null });
    if (isMobileViewport) setExplicitPanelSize("min");
  };
  ...
  // ─── Canada census-divisions drill-down ──────────────────────────
  const handleDoubleClickProvince = (geoId: string) => {
    // geoId is like "CA-AB". Find province name from the geoId.
    const provName = Object.entries(PROVINCE_GEOID_MAP).find(
      ([, id]) => id === geoId,
    )?.[0];
    if (provName) {
      stageCdDrill(geoId, provName);
    }
  };

  const CD_DRILL_LIFT_MS = 360;
  const stageCdDrill = (geoId: string, provName: string) => {
    setDrillingTo(geoId);
    if (isMobileViewport) setExplicitPanelSize("min");
    window.setTimeout(() => {
      handleViewCensusDivisions(provName);
      requestAnimationFrame(() => setDrillingTo(null));
    }, CD_DRILL_LIFT_MS);
  };

  const handleViewCensusDivisions = (provName: string) => {
    const uid = PROVINCE_UID[provName];
    if (!uid) return;
    navigateTo({
      region: "na",
      naView: "census-divisions",
      selectedGeoId: null,
      selectedProvinceUid: uid,
  ...
  const handleDoubleClickEntity = (geoId: string) => {
    ...
    if (geoId === "124") handleViewProvinces();
  ...
  const breadcrumbItems: BreadcrumbItem[] = useMemo(() => {
    ...
    } else if (region === "na" && naView === "provinces") {
      items.push({
        label: "Canada",
        onClick: selectedGeoId ? handleViewProvinces : undefined,
      });
      if (selectedGeoId && selectedEntity && !selectedEntity.isOverview) {
        items.push({ label: selectedEntity.name });
      }
    } else if (region === "na" && naView === "census-divisions" && selectedProvinceName) {
      items.push({
        label: "Canada",
        onClick: handleViewProvinces,
      });
      items.push({
        label: selectedProvinceName,
        onClick: selectedCduid ? handleResetCds : undefined,
      });
      if (selectedCduid) {
        const cd = getCensusDivisionByUid(selectedCduid);
        if (cd) items.push({ label: cd.name });
      }
    } else if (
      selectedGeoId &&
      selectedEntity &&
  ...
  const onMapBackgroundClick = (e: React.MouseEvent) => {
    ...
    if (region === "na" && naView === "census-divisions") {
      handleViewProvinces();
      return;
    }
    ...
    const drillOut = () => {
      ...
      if (region === "na" && naView === "census-divisions") {
        navigateTo({
          region: "na",
          naView: "provinces",
          selectedGeoId: selectedProvinceName
            ? PROVINCE_GEOID_MAP[selectedProvinceName] ?? null
            : null,
        });
        return true;
      }
  ...
  const NA_DEPTH: Record<NaView, number> = useMemo(
    () => ({ countries: 0, states: 1, provinces: 1, counties: 2, "census-divisions": 2 }),
    [],
  );
    ...
    <div ref={mapRootRef} className="fixed inset-0 bg-white overflow-hidden z-0">
      ...
      >
        ...
        >
          ...
          >
            {r === "na" && (
              <div
                key={`na-${naView}-${selectedStateName ?? ""}-${selectedProvinceName ?? ""}`}
              ...
              >
                {naView === "countries" && (
                  <NorthAmericaMap
                    ...
                    onSelectCanada={handleViewProvinces}
                ...
                {naView === "provinces" && (
                  <CanadaProvincesMap
                    onSelectEntity={handleSelectProvince}
                    onDoubleClickEntity={handleDoubleClickProvince}
                    selectedGeoId={selectedGeoId}
                    setTooltip={setTooltip}
                    dimension={dimension}
                    lens={lens}
      ...
      {tooltip && !hoveredProject && (() => {
        ...
        const activeLensDims = (
          lens === "affordability" ? AFFORDABILITY_DIMENSIONS : ZONING_DIMENSIONS
        ) as Exclude<Dimension, "overall">[];
        const fallbackLensDims = (
          lens === "affordability" ? ZONING_DIMENSIONS : AFFORDABILITY_DIMENSIONS
        ) as Exclude<Dimension, "overall">[];
        ...
        return (
          ...
          >
            ...
            >
              {/* Type scale (shared with ProjectCard):
...
/**
 * One band in the project size key. A dot at the map's actual pixel
 * radius paired with the unit-count bucket label. The three bands
 * mirror `SIZE_BANDS` in ProjectDots so the map key and the visual
 * dots on the map stay in lock-step.
 */
